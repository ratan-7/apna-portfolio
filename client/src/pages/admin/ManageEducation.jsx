...
export default function ManageEducation() {
    ...
    const handleDelete = async (id) => {
        if (!confirm("Ye education entry delete karni hai?")) return;
        try {
        ...
        <div>
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">Apni education history manage karo.</p>
                </div>
                <button
                    onClick={openAddForm}
            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="mt-5 rounded-lg border border-gray-200 bg-white p-5">
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <input name="school" required placeholder="School / College" value={form.school} onChange={handleChange}
                            className="rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500" />
                        <input name="degree" required placeholder="Degree (e.g. B.Tech)" value={form.degree} onChange={handleChange}
                            className="rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500" />
                        <input name="field" placeholder="Field of Study" value={form.field} onChange={handleChange}
                            className="rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500" />
                        <input name="grade" placeholder="Grade / CGPA" value={form.grade} onChange={handleChange}
                            className="rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500" />
            ...
            <div className="mt-5 flex flex-col gap-3">
                {loading ? (
                    ...
                    <p className="text-sm text-gray-400">Koi education entry nahi hai abhi.</p>
                ) : (
                    items.map((item) => (
                        <div key={item._id} className="flex items-start justify-between rounded-lg border border-gray-200 bg-white p-4">
                            <div>
                                <h3 className="font-medium text-gray-900">
                                    {item.degree} <span className="text-gray-400">— {item.school}</span>
                                </h3>
                                <p className="mt-0.5 text-xs text-gray-500">
                                    {item.field} · {item.startDate?.slice(0, 10)} —{" "}
